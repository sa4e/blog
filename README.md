...
到了这里blog**0.1**版本的业务功能已全部开发完成！从8月14日开始搭建环境到前台模型编写到数据库建表再到服务端的编写整个历时将近1个半月,着实不易。接下来就轻松很多了,功能测试基本都在开发过程中就通过了,然后再花两天时间把云服务器上的运行环境搭建好和再把生产环境的配置搞好就上线**0.1**版本啦!  
